...
public enum ComponentModelQuery {
	...
	private List<String> sourcesWithoutFQN = new ArrayList<String>();
	...
	private void createAppLevelComponent(List<Component> componentList){
		...
		if (topLevelComponents.size() > 1){
			String appName = model.getPackagedElements().get(0).getName();
			appLevelComponent = new Component("APP", false, true, appName, new ArrayList<String>(), new ArrayList<String>(), new ArrayList<MigrationStrategy>(), new ArrayList<String>()); 
	...
	public List<Component> analyzeAllComponents() {
		...
		ksession.setGlobal("goals", MigrationGoalsContainer.INSTANCE.getMigrationGoals());
		...
		ksession.getAgenda().getAgendaGroup(MigrationGoalsContainer.INSTANCE.getMigrationGoals().getTargetPlatform().name()).setFocus();
		...
		//Moved to the inventory view, triggered by Open Efforts View button
	...
	/**
	 * Re-assigns migration strategies to the passed list of components 
	 * @return A {@link List} of analyzed components
	 */
	public List<Component> reAssignMigrationStrategies (List<Component> componentList) {
		...
		for (PackageableElement p : uml2Component.getPackagedElements()){
			if (p instanceof Class && !(p instanceof org.eclipse.uml2.uml.Component)) {
				...
				if (((Class) p).getAttribute("qualifiedName", null) != null){
					String qualifiedName = ((Class) p).getAttribute("qualifiedName", null).getDefault();
					if (qualifiedName.trim().equals("")){
						sourcesWithoutFQN.add(((Class) p).getName());
					}
					else {
						ownedClassesWithFQN.add(qualifiedName.replace("::", "/"));
					}
				}
				else {
					sourcesWithoutFQN.add(((Class) p).getName());
	...
	public List<String> getSourcesWithoutFQN(){
		return sourcesWithoutFQN;
	}

	public String getSourcesWithoutFQNasText(){
		StringBuffer returnValue = new StringBuffer();

		for (String source : sourcesWithoutFQN) {
			returnValue.append(source).append(" ");
		}
