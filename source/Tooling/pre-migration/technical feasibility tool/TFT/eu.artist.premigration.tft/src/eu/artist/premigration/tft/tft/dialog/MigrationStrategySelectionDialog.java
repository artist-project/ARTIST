...
public class MigrationStrategySelectionDialog extends Dialog{
	...
	private List <MigrationStrategy> previousStrategies;

	private boolean okPressed = false;

	private StatefulKnowledgeSession ksession;
	private KnowledgeBase kbase;
	private static final String RULE_PATH = "eu/artist/premigration/tft/tft/rules/GetAllAvailableStrategies.drl";
	...
	public MigrationStrategySelectionDialog (Shell parentShell, Component selectedComponent) {
		...
		previousStrategies = new ArrayList<MigrationStrategy>();
		for (MigrationStrategy strategy : selectedComponent.getMigrationStrategies()) {
			previousStrategies.add(strategy);
		}
	...
	protected Control createDialogArea(Composite parent) {
		...
		final Text description = new Text(container, SWT.MULTI | SWT.WRAP);
		description.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_BACKGROUND));
		description.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 3, 5));
		...
		availableStrategiesTree.addSelectionChangedListener(new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (!selection.isEmpty() && selection.getFirstElement() instanceof ChildStrategy){
					...
					String descriptionText = selectedStrategy.getStrategy().getDescription();
					description.setText((descriptionText != null && !descriptionText.trim().equals("")) ? descriptionText : "No description available yet.");
				}
				else {
...
//		ksession.insert(MigrationGoalsContainer.INSTANCE.getMigrationGoals());
	...
	public boolean close() {
		if (!okPressed){
			selectedComponent.setMigrationStrategies(previousStrategies);
		}
		return super.close();
	}
