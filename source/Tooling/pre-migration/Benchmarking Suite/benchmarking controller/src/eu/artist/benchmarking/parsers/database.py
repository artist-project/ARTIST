...
def database(Date,CloudProvider,User,SIZE_ID,Tool,Workload,Platform,a,b,c,d,e,f,g,h1,i,l,m,n,o,p,q,r,s,t,u,v,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12):
    if Tool == 'Filebench':
        mydb = MySQLdb.connect(host='127.0.0.1',user='root',passwd='test',db='cloudbench')
        cursor = mydb.cursor()
        cursor.execute('INSERT INTO cloudbench.Filebench(Date,CloudProvider,User,InstanceType,Tool,Workload,Platform,Ops,OpsPerSecond,ReadWrite,MbPerSecond,CpuOperations,Latency) VALUES (%s,%s,%s,%s,%s,%s,%s,"%r","%r",%s,"%r","%r","%r")',(st,CloudProvider,User,SIZE_ID,Tool,Workload,Platform,a,b,c,d,e,f))
        mydb.commit()
        cursor.close()
        print "data stored!!"
    else:
        if Tool == 'DaCapo':
            mydb = MySQLdb.connect(host='127.0.0.1',user='root',passwd='test',db='cloudbench')
            cursor = mydb.cursor()
            cursor.execute('INSERT INTO cloudbench.DaCapo(Date,CloudProvider,User,InstanceType,Tool,Workload,Platform,PerformanceTime_ms) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)',(st,CloudProvider,User,SIZE_ID,Tool,Workload,Platform,a))
            mydb.commit()
            cursor.close ()
            print "data stored!!"
        else:
