...
public class AWSEC2SLAAuditorTEMPLATE implements AuditingInterface {
	...
	public  void checkApplicabilityConditions(String provider,String user,String apiKey, String databaseIP, String DBuser, String DBkey) throws Exception {
		...
		for (String validTemplate: validTemplateIDs){
			...
			for (NodeMetadata nodeinfo : validNodeIDs) {
				if ((nodeinfo.getImageId()).equals(validTemplate)){
					...
					if (checkReachability(nodeinfo)){
						...
						System.out.println("Node:"+nodeinfo.getId()+" of template:"+validTemplate+" is reachable");
					}else {System.out.println("Node:"+nodeinfo.getId()+" of template:"+validTemplate+" is not reachable");}
				}
			}
