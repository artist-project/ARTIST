...
/**
 * Generated implementation of the Henshin unit "ParallelTriangles".
 */
@Algorithm(
    name = "ParallelTriangles"
)
public class ParallelTriangles extends
  ...
  /**
   * Rule constant for "ParallelTriangles".
   */
  public static final int RULE_PARALLEL_TRIANGLES = 0;

  /**
   * Logging support.
   */
  protected static final Logger LOG = Logger.getLogger(ParallelTriangles.class);
  ...
  public void compute(
    ...
    switch (rule) {
    case RULE_PARALLEL_TRIANGLES:
      matchParallelTriangles(
  ...
  /**
   * Match (and apply) the rule "ParallelTriangles".
  ...
  protected void matchParallelTriangles(
    ...
    matches = filterParallelTriangles(
    ...
    if (microstep == 0) {
      ...
      if (ok) {
        for (Match match : matches) {
          ...
          // Node "c": check for edge to match of "b" of type "left":
  ...
  /**
   * Filter matches per segment for the rule "ParallelTriangles".
   * @param vertex The current vertex.
   * @param matches The current matches.
   * @param segment The current segment.
   * @param microstep The current microstep.
   ...
   */
  protected Iterable<Match> filterParallelTriangles(
    Vertex<VertexId, ByteWritable, ByteWritable> vertex,
    Iterable<Match> matches, int segment, int microstep,
      ...
      for (Match match : matches) {
        ...
        if (matchSegment < segment) {
          ...
          throw new RuntimeException("Received match " + match +
            " of rule ParallelTriangles of segment " +
  ...
  /**
   * Apply the rule "ParallelTriangles" to a given match.
   ...
   */
  protected boolean applyParallelTriangles(
    Vertex<VertexId, ByteWritable, ByteWritable> vertex,
    ...
    LOG.info("Vertex " + vertex.getId() +
      " applying rule ParallelTriangles with match " + match);
  ...
  public static class MasterCompute extends DefaultMasterCompute {
    ...
    public void compute() {
      ...
      if (getSuperstep() == 0) {
        ...
        stack = stack.append(RULE_PARALLEL_TRIANGLES, 0, 0);
      } else {
    ...
    private ApplicationStack nextRuleStep(
      ...
      while (stack.getStackSize() > 0) {
        ...
        switch (unit) {
        case RULE_PARALLEL_TRIANGLES:
          stack = processParallelTriangles(
        ...
        if (stack.getStackSize() > 0) {
          ...
          if (unit == RULE_PARALLEL_TRIANGLES) {
            break;
          }
        }
      }
   ...
   /**
     * Process Rule "ParallelTriangles".
     * @param stack The current application stack.
     * @param segment The current segment.
     * @param microstep The current microstep.
     * @param ruleApps Number of rule applications in last superstep.
     * @return The new application stack.
     */
    private ApplicationStack processParallelTriangles(
      ApplicationStack stack, int segment, int microstep, long ruleApps) {
      if (microstep < 3) {
        stack = stack.append(RULE_PARALLEL_TRIANGLES, segment, microstep + 1);
      } else if (segment < SEGMENT_COUNT - 1) {
        stack = stack.append(RULE_PARALLEL_TRIANGLES, segment + 1, 0);
      } else {
