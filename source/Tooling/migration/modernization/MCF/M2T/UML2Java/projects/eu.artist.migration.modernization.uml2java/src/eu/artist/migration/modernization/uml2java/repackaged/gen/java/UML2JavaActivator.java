...
package eu.artist.migration.modernization.uml2java.repackaged.gen.java;
...
public class UML2JavaActivator extends Plugin {
     ...
     * The plug-in ID.
     */
    public static final String PLUGIN_ID = "eu.artist.migration.modernization.uml2java";
	...
	public static void log(Exception e, boolean blocker) {
		...
		if (getDefault() == null) {
			...
			log(new Status(severity, PLUGIN_ID, severity, UML2JavaMessages.getString("UML2JavaPlugin.RequiredElementNotFound"), e)); //$NON-NLS-1$
		} else {
			int severity = IStatus.WARNING;
			if (blocker) {
				severity = IStatus.ERROR;
	 ...
	 * Log an information.
	 * 
	 * @param information the message to log.
	 */
