...
public class ImportServices {
	 ...
	 */
	public static final String END_IMPORT = ";" + System.getProperty("line.separator");
	...
	public String reqImport(Classifier aClassifier) {
		...
		importedTypesToRemove.clear();
		String qualifiedName = this.reqQualifiedName(aClassifier, false);
	...
	private Set<String> typeToImports(Classifier aClassifier) {
		...
		for (Operation operation : operations) {
			...
			for (Type type : raisedExceptions) {
				String exceptionQualifiedName = this.reqQualifiedName(type, ignoreJavaTypes);
		...
		for (Generalization generalization : generalizations) {
			importedTypes.add(this.reqQualifiedName(generalization.getGeneral(), ignoreJavaTypes));
		}
		...
		if (aClassifier instanceof org.eclipse.uml2.uml.Class) {
			...
			for (InterfaceRealization interfaceRealization : interfaceRealizations) {
				importedTypes.add(this.reqQualifiedName(interfaceRealization.getContract(), ignoreJavaTypes));
			}
		}
	 ...
	 */
	public String reqQualifiedName(Type type, boolean ignoreJavaTypes) {
		String result = null;
