...
import eu.artist.postmigration.nfrvt.eval.run.internal.process.InternalGMLProcess;
import eu.artist.postmigration.nfrvt.eval.run.logger.ConsoleLogger;


public class GMLProcess extends PlatformObject implements IProcess {

	private ILaunch launch;
	private InternalGMLProcess internalProcess;
	private String name;
	...
	public GMLProcess(ILaunch launch, Process process, String name,
	...
	private void setFields(ILaunch launch, Process process, String name,
			@SuppressWarnings("rawtypes") Map attributes) {
		this.launch = launch;
		assertGMLProcess(process);
		this.internalProcess = (InternalGMLProcess) process;
		launch.addProcess(this);
		this.name = name;
		this.attributes = attributes;
	}

	private void assertGMLProcess(Process process) {
		Assert.isTrue(process instanceof InternalGMLProcess);
	}
	...
	public int getExitValue() throws DebugException {
		return InternalGMLProcess.EXIT_VALUE;
	}
