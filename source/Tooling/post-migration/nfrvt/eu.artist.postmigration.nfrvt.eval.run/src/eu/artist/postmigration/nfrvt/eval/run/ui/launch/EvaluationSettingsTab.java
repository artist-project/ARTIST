...
public class EvaluationSettingsTab extends AbstractLaunchConfigurationTab {
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy configuration) {
		configuration.setAttribute(MigrationEvaluationActivator.ATT_SETTINGS_PRECISION, defaultPrecision);
		configuration.setAttribute(MigrationEvaluationActivator.ATT_SETTINGS_ROUNDING, defaultRoundingMode.name());
	}

	@Override
	public void initializeFrom(ILaunchConfiguration configuration) {
		try {
			int precision = configuration.getAttribute(MigrationEvaluationActivator.ATT_SETTINGS_PRECISION, defaultPrecision);
			if(precisionText != null)
				precisionText.setText(Integer.toString(precision));
			String roundingModeName = configuration.getAttribute(MigrationEvaluationActivator.ATT_SETTINGS_ROUNDING, defaultRoundingMode.name());
	...
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {
		configuration.setAttribute(DebugPlugin.ATTR_PROCESS_FACTORY_ID, MigrationEvaluationActivator.GML_PROCESS_FACTORY_ID);
		configuration.setAttribute(MigrationEvaluationActivator.ATT_SETTINGS_PRECISION, getPrecision());
		configuration.setAttribute(MigrationEvaluationActivator.ATT_SETTINGS_ROUNDING, getRoundingMode().name());
	}
	...
	public Image getImage() {
		URL imgUrl = MigrationEvaluationActivator.getDefault().getBundle().getEntry("icons/EvaluationSettings.gif");
		if(imgUrl != null) {
			ImageDescriptor id = null;
			Image result = MigrationEvaluationActivator.getDefault().getImageRegistry().get(imgUrl.toExternalForm());
			if (result == null) {
				id = ImageDescriptor.createFromURL(imgUrl);
				if (id != null) {
					result = id.createImage();
					MigrationEvaluationActivator.getDefault().getImageRegistry().put(imgUrl.toExternalForm(), result);
				}
			}
			return result;
		}
